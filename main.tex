...
By shifting the focus from the mechanics of extraction to its economic incidence, this paper contributes to the growing literature on blockchain market microstructure and the broader consequences of financial innovation.
	...
	The sandwich tax is regressive, in two senses: (a) \emph{attack incidence} -- attack events fall overwhelmingly on small trades, which account for a far larger share of attacks than of attacked volume; and (b) \emph{loss incidence} -- the loss per dollar traded declines with trade size. Part (a) is directly observable in our data; part (b) is the tax-rate sense that the title invokes, and testing it requires either an explicit assumption on how per-attack losses scale with trade size or realised-slippage microdata.
	\end{hypothesis}
    ...
    however, Uniswap alone accounts for approximately 86\% of all sandwich
    events in the sample. Valuation errors may therefore have a greater
...
tiers, motivating the three-way classification used throughout the analysis.}
    ...
    The \$247.3B headline figure measures capital \emph{exposed} to
    ...
    Uniswap alone accounts for approximately 86\% of all sandwich events
    in our sample. Such valuation error may therefore have a greater
    effect on individual protocols or tokens than on the aggregate event
    count.
