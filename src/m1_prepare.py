...
def prepare_all():
    ...
    total_victim_events = q5b["victim_count"].sum()
    tier_addr_col = m0_validate.q5b_tier_address_col(q5b)
    q5b["attacks_per_victim"] = q5b["victim_count"] / q5b[tier_addr_col]
    q5b["attacks_per_1000usd"] = (q5b["victim_count"] / q5b["total_volume"]) * 1000
    q5b["share_of_events"] = q5b["victim_count"] / total_victim_events
    q5b["share_of_volume"] = q5b["total_volume"] / q5b["total_volume"].sum()
    q5b["avg_volume_per_tier_address_obs"] = q5b["total_volume"] / q5b[tier_addr_col]
    ...
    end_threshold = max_date.replace(day=1)
    B_pos["active_at_end"] = (B_pos["last_seen_parsed"] >= end_threshold).astype(int)
    ...
    B_meas["active_at_end"] = (B_meas["last_seen_parsed"] >= end_threshold).astype(int)
    ...
    tex_lines = [
        "\\begin{table}[htbp]",
        "\\centering",
        f"\\caption{{Summary Statistics of the Sandwich-MEV Dataset ({date_window_label})}}",
        "\\label{tab:descriptives}",
        "\\begin{tabular}{lrrrrrrrr}",
        "\\toprule",
        "Variable & N & Mean & Std & Min & P25 & Median & P75 & Max \\\\",
        "\\midrule",
        f"\\multicolumn{{9}}{{l}}{{\\textbf{{Panel A: Daily Bot-Side Series (N = {len(q1):,} days)}}}} \\\\",
        "\\midrule"
    ]
    ...
    bot_trade_total = int(q4["total_sandwich_trades"].sum())
    bot_volume_total_b = q4["total_volume_usd"].sum() / 1e9
    victim_volume_total_b = q5b["total_volume"].sum() / 1e9
    victim_addr_total = int(q5b[tier_addr_col].sum())
    b_meas_n = int(B_meas.shape[0])
    b_pos_n = int(B_pos.shape[0])
    min_trade_val = q5a.loc[0, "min_value"]
    notes_str = (
        f"This table presents descriptive statistics for the Ethereum Sandwich-MEV dataset ({date_window_label}). "
        f"\\textbf{{Measurement bases must not be mixed without adjustment:}} Panels A and B report on the \\textbf{{bot-side base}}, comprising {bot_trade_total:,} sandwich attacker trade legs and \\${bot_volume_total_b:,.2f}B in total notional volume across {len(q4):,} bot addresses ({b_meas_n:,} with non-missing volume in $B_{{\\text{{meas}}}}$, and {b_pos_n:,} with strictly positive volume in $B_{{\\text{{pos}}}}$). "
        f"Panel C reports on the \\textbf{{victim-side base}}, comprising {int(total_victim_events):,} sandwiched victim trade events (attack events) and \\${victim_volume_total_b:,.2f}B in total volume across {victim_addr_total:,} tier-address observations. "
        "In Panel C, N represents the number of sandwiched victim trades (attack events), not unique addresses. "
        f"The minimum trade size in Panel C ({min_trade_val:.2e} USD) represents an extreme pricing/precision artifact in raw DEX logs."
    )
